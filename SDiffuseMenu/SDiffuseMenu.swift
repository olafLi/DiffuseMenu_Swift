...
open class SDiffuseMenu : UIView, SDiffuseMenuItemDelegate, CAAnimationDelegate {
    ...
    /// delegate
    open var delegate:               SDiffuseMenuDelegate!
    
    /// 动画的类型:默认弧线动画
    open var sDiffuseMenuGrapyType:  SDiffuseMenuGrapyType = .arc {
        didSet {
    ...
    /// 动画的方向:方向默认右上角
    open var sDiffuseMenuDirection: SDiffuseMenuDirection = .upperRight {
        didSet {
    ...
    public init(frame: CGRect, startItem: SDiffuseMenuItem, menusArray: NSArray, grapyType: SDiffuseMenuGrapyType) {
    ...
    public func SDiffuseMenuItemTouchesBegan(_ item: SDiffuseMenuItem) {
    ...
    public func SDiffuseMenuItemTouchesEnd(_ item: SDiffuseMenuItem) {
        if (item == _startButton) {
            return
        }
